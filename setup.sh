#!/bin/bash

# ===========================================
# Neetrino Platform - Setup Script
# ===========================================
# Скрипт для настройки проекта на новом компьютере

set -e  # Остановить выполнение при ошибке

echo "🚀 Настройка проекта Neetrino Platform..."

# ===========================================
# Проверка Node.js версии
# ===========================================
echo "📋 Проверка версии Node.js..."

if command -v nvm &> /dev/null; then
    echo "✅ NVM найден, используем версию из .nvmrc"
    nvm use
else
    echo "⚠️  NVM не найден, проверяем версию Node.js"
    NODE_VERSION=$(node --version)
    echo "Текущая версия Node.js: $NODE_VERSION"
    
    if [[ "$NODE_VERSION" != "v18."* ]]; then
        echo "❌ Требуется Node.js версии 18.x.x"
        echo "Установите Node.js 18.x.x или используйте NVM"
        exit 1
    fi
fi

# ===========================================
# Установка зависимостей
# ===========================================
echo "📦 Установка зависимостей..."

# Установка корневых зависимостей
echo "Установка корневых зависимостей..."
npm install

# Установка зависимостей для каждого приложения
echo "Установка зависимостей для API..."
cd apps/api && npm install && cd ../..

echo "Установка зависимостей для Web..."
cd apps/web && npm install && cd ../..

echo "Установка зависимостей для Types..."
cd packages/types && npm install && cd ../..

echo "Установка зависимостей для UI..."
cd packages/ui && npm install && cd ../..

# ===========================================
# Настройка переменных окружения
# ===========================================
echo "⚙️  Настройка переменных окружения..."

if [ ! -f .env ]; then
    if [ -f env.example ]; then
        echo "Создание .env файла из примера..."
        cp env.example .env
        echo "✅ Файл .env создан из env.example"
        echo "⚠️  Не забудьте настроить переменные в .env файле!"
    else
        echo "❌ Файл env.example не найден"
    fi
else
    echo "✅ Файл .env уже существует"
fi

# ===========================================
# Сборка проекта
# ===========================================
echo "🔨 Сборка проекта..."

# Сборка пакетов
echo "Сборка пакетов..."
npm run build

# ===========================================
# Проверка установки
# ===========================================
echo "✅ Проверка установки..."

# Проверка что все приложения собираются
echo "Проверка API..."
cd apps/api && npm run build && cd ../..

echo "Проверка Web..."
cd apps/web && npm run build && cd ../..

echo "Проверка Types..."
cd packages/types && npm run build && cd ../..

echo "Проверка UI..."
cd packages/ui && npm run build && cd ../..

# ===========================================
# Завершение
# ===========================================
echo ""
echo "🎉 Настройка завершена!"
echo ""
echo "📋 Следующие шаги:"
echo "1. Настройте переменные в файле .env"
echo "2. Настройте базу данных PostgreSQL"
echo "3. Запустите проект: npm run dev"
echo ""
echo "🔗 Полезные команды:"
echo "  npm run dev     - Запуск в режиме разработки"
echo "  npm run build   - Сборка проекта"
echo "  npm run lint    - Проверка кода"
echo "  npm run clean   - Очистка проекта"
echo ""
echo "📚 Документация: README.md"
